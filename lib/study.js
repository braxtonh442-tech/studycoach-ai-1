...
async function updateStudentMemory(userId, message, subject, answer) {
  try {
    ...
    if (
      ...
      learningStyle = "Learns well with examples";
    }
    ...
    if (
      ...
      learningStyle = "Step-by-step learner";
    }

    const updates = {
      recently_learned: subject || "Study",
      memory_notes: `Recently asked about: ${String(message).slice(0,120)}`,
      last_ai_summary: String(answer || "").slice(0,300),
      updated_at: new Date().toISOString()
    };

    if (learningStyle) {
      updates.learning_style = learningStyle;
    }
