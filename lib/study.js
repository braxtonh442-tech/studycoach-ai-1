...
async function checkAchievements(userId) {
  ...
  const chatCount = tasks.filter(x => x.type === "chat").length;
  const quizCount = tasks.filter(x => x.type === "quiz-answer").length;
  const flashCount = tasks.filter(x => x.type === "flashcards").length;
  const homeworkCount = tasks.filter(x => x.type === "homework").length;
  ...
  if (quizCount >= 1) {
    await unlockAchievement(
      ...
      "first-quiz-answer",
      "First Quiz Answer",
      "Answered your first quiz question.",
      "🧠"
    );
  }
...
const { data: profile } = await supabase
  ...
  .from("users")
  .select("plan, trial_ends_at, free_questions_used")
  .eq("id", userId)
  .maybeSingle();
...
const unlimited =
  user?.plan === "premium" || hasTrial;

if (!unlimited && (user?.free_questions_used || 0) >= 10) {
  return res.status(403).json({
    error:
      "You've used your 50 free AI questions. Upgrade to Premium for unlimited study help."
  });
}
    if (!message) {
...
await updateStudentMemory(
  ...
  await supabase
    .from("users")
    .update({
      free_questions_used: (user.free_questions_used || 0) + 1
    })
    .eq("id", userId);
}
    res.json({
...
const studyDates = Array.from(days).sort();
    let currentStreak = 0;
let bestStreak = 0;
...
if (studyDates.length) {
  ...
  const lastStudy = new Date(studyDates[studyDates.length - 1]);
  const diffToday = Math.round((today - lastStudy) / (1000 * 60 * 60 * 24));
...
const claimedToday = progress.some(p =>
  p.type === "daily-reward" &&
  String(p.created_at || "").slice(0, 10) === todayKey
);
    res.json({        
      claimedToday,
      currentStreak,
...
studyRouter.post("/claim-daily-reward", async (req, res) => {
  try {
    const userId = getUserId(req);
    const todayKey = new Date().toISOString().slice(0, 10);
    ...
    const alreadyClaimed = (progress || []).some(p =>
      p.type === "daily-reward" &&
      String(p.created_at || "").slice(0, 10) === todayKey
    );
    ...
    res.status(500).json({
      error: "Daily reward error: " + err.message
    });
  }
});
...
if (cards.length === 0) {
  cards = [
    {
      front: `What is ${topic}?`,
      back: `A key idea in ${subject}.`
    },
    {
      front: `Give an example of ${topic}.`,
      back: `Example depends on the topic.`
    },
    {
      front: `Why is ${topic} important?`,
      back: `It helps you understand ${subject}.`
    }
  ];
}
     await supabase.from("progress").insert({
...
studyRouter.post("/study-plan", async (req, res) => {
  try {
    ...
    const goal = req.body.goal || "Improve my studies";
    const subject = req.body.subject || "Study";
...
`
      ...
      const lines = ai.answer
        .split("\n")
        .filter(x => x.trim().startsWith("Day"));

      planDays = lines.map((line, i) => ({
        day: i + 1,
        task: line.replace(/^Day\s*\d+\s*:\s*/i, "").trim(),
        minutes: 20
      }));
    }
    ...
    if (planDays.length === 0) {
      planDays = Array.from({ length: days }, (_, i) => ({
        day: i + 1,
        task: `Study ${subject} for today's goal: ${goal}`,
        minutes: 20
      }));
    }
    ...
    res.status(500).json({
      error: "Study plan error: " + err.message
    });
  }
});
studyRouter.post("/conversations", async (req, res) => {
  try {
    const userId = getUserId(req);
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        user_id: userId,
        title: "New chat",
        subject: req.body.subject || "Study"
      })
      .select()
      .single();
    ...
    res.status(500).json({
      error: "Create conversation error: " + err.message
    });
  }
});
studyRouter.get("/conversations", async (req, res) => {
  try {
    const userId = getUserId(req);
    ...
    res.status(500).json({
      error: "Load conversations error: " + err.message,
      conversations: []
    });
  }
});

studyRouter.get("/conversations/:id/messages", async (req, res) => {
  try {
    const userId = getUserId(req);
    ...
    res.status(500).json({
      error: "Load messages error: " + err.message,
      messages: []
    });
  }
});
studyRouter.delete("/conversations/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Delete conversation error: " + err.message
    });
